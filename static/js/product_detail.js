(function () {
  // galleryData comes from template via window.galleryData
  ...
  function showImage(idx) {
    ...
    if (counter) {
      counter.textContent =
        String(currentIdx + 1) + " / " + String(galleryData.length);
    }
  }

  // Open modal when clicking main image or any thumbnail
  document.addEventListener("click", (e) => {
    const trigger = e.target.closest(
      '[data-bs-target="#imageModal"][data-idx]'
    );
    if (!trigger) return;
