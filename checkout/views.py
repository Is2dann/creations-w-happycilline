...
# They remain in the code as they are useful later too.
...
def checkout(request):
    ...
    amount = int(
        grand_total
        * int(getattr(settings, "STRIPE_PRICE_MULTIPLIER", 100))
    )
...
def cache_checkout_data(request):
    """
    Saves checkout form to session and attaches minimal data to the payment
    intent metadata so the webhook can build the order without the browser
    returning.
    """
    stripe.api_key = settings.STRIPE_SECRET_KEY
    data = request.POST
    ...
    if client_secret:
        ...
        meta = {
            ...
            "country": data.get("country", ""),
            "save_info": (
                "true" if data.get("save_info") else "false"
            ),
    ...
    # Tiny 200 template used as the inline fetch() response
...
def stripe_webhook(request):
    ...
    try:
        ...
        logger.warning(
            "Stripe webhook signature/payload error: %s",
            e,
        )
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "Order %s created; email failed: %s",
                order.order_number,
                e,
            )
...
def checkout_paid(request):
    ...
    - Otherwise wait briefly; if still not found, create it here from
      PaymentIntent metadata/session.
    """
    stripe.api_key = settings.STRIPE_SECRET_KEY
    ...
    if bag_from_meta:
        ...
        (
            ...
            delivery_cost,
            grand_total,
        ) = _summary_from_bag_dict(bag_from_meta)
        user_profile = None
        ...
        if created:
            ...
            try:
                ...
                logger.exception(
                    "Order %s created via return-url; email failed: %s",
                    order.order_number,
                    e,
                )
    else:
        ...
        if created:
            ...
            try:
                ...
                logger.exception(
                    (
                        "Order is created via return-url; "
                        "email failed: %s"
                    ),
                    order.order_number,
                    e,
...
def order_detail(request, order_number):
    ...
    if not request.user.is_staff:
        if (
            not order.user_profile
            or order.user_profile.user_id != request.user.id
        ):
